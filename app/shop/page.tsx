...
export default async function ShopPage({ searchParams }: { searchParams: Promise<{ category?: string; q?: string }> }) {
  ...
  let query = supabase.from('products').select('*, categories(name, id)').eq('active', true).order('featured', { ascending: false }).order('created_at', { ascending: false })
  ...
  return (
    <div className="pb-4">
      {/* Banner / Destaque - só aparece sem filtros */}
      ...
      {!params.q && (
        <div className="flex items-center justify-between px-4 pt-2 pb-3">
          <h2 className="font-black text-slate-900 text-base" style={{ fontFamily: 'Arial Black, sans-serif' }}>
            {params.category
              ? categories?.find((c: any) => c.id === params.category)?.name || 'Produtos'
              : 'Todos os Produtos'}
          </h2>
      ...
      {/* Grid de produtos - 2 colunas estilo marketplace */}
      {products && products.length > 0 ? (
            ...
            <div key={product.id} className="bg-white rounded-2xl overflow-hidden border border-slate-100 flex flex-col">
              ...
              <div className="p-3 flex flex-col flex-1">
                <Link href={`/shop/products/${product.slug}`}>
                  <p className="text-xs text-slate-900 font-medium leading-tight line-clamp-2 mb-2 min-h-[2.5rem]">
                    {product.name}
                  </p>
                </Link>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <p className="text-sm text-slate-400 text-center mt-1">Tente ajustar os filtros</p>
