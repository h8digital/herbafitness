...
const navItems = [
  {
    href: '/shop', label: 'Início', exact: true,
    icon: (active: boolean) => (
  ...
  {
    href: '/shop/categories', label: 'Categorias',
    icon: (active: boolean) => (
      <svg className="w-6 h-6" fill={active ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 0 : 1.8}>
  ...
  },
  {
    ...
    icon: (active: boolean) => (
      <svg className="w-6 h-6" fill={active ? '#ef4444' : 'none'} viewBox="0 0 24 24"
        stroke={active ? '#ef4444' : 'currentColor'} strokeWidth={active ? 0 : 1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
      </svg>
    ),
  },
  {
    href: '/shop/cart', label: 'Carrinho', isCart: true,
    icon: (active: boolean) => (
      <svg className="w-6 h-6" fill={active ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 0 : 1.8}>
  ...
  {
    href: '/shop/profile', label: 'Perfil',
    icon: (active: boolean) => (
      <svg className="w-6 h-6" fill={active ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 0 : 1.8}>
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              {isActive && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full" style={{ background: '#1B5E20' }} />}
            </Link>
          )
        })}
      </div>
