...
export default function CartDrawer({ open, onClose }: CartDrawerProps) {
  const { items, removeItem, updateQuantity, subtotal, couponDiscount, selectedShipping, itemCount } = useCartStore()
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: '#e8f5e9' }}>
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-lg" style={{ color: '#1B5E20', fontFamily: 'Arial Black, sans-serif' }}>
              Carrinho
            </h2>
            {itemCount > 0 && (
              <span className="text-white text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: '#4CAF50' }}>
                {itemCount}
              </span>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              ...
              <button
                onClick={onClose}
                className="text-white font-semibold px-6 py-2.5 rounded-xl transition-colors text-sm"
            ...
            <div className="space-y-4">
              {items.map(item => (
                <div key={item.product.id} className="flex gap-3 p-3 rounded-2xl border" style={{ borderColor: '#e8f5e9' }}>
                  {/* Imagem */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="font-bold text-sm mt-0.5" style={{ color: '#2E7D32' }}>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex items-center rounded-xl overflow-hidden border" style={{ borderColor: '#c8e6c9' }}>
                        <button
                          ...
                          className="w-7 h-7 flex items-center justify-center text-sm font-bold transition-colors hover:opacity-70"
                          style={{ color: '#1B5E20' }}
                        >−</button>
                        <span className="w-7 h-7 flex items-center justify-center text-sm font-semibold" style={{ color: '#1B5E20' }}>
                          {item.quantity}
                        </span>
                        <button
        ...
        {items.length > 0 && (
          <div className="px-6 py-4 border-t space-y-3" style={{ borderColor: '#e8f5e9', background: '#fafffe' }}>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-lg" style={{ color: '#1B5E20' }}>{formatCurrency(total)}</span>
            </div>
            ...
            <Link
              href="/shop/checkout"
              onClick={onClose}
              className="block w-full text-center text-white font-bold py-3.5 rounded-xl transition-all text-sm"
